...
//!
//! Instead of using the form `[-o, +p, x]`, zslua uses the words **Pops**, **Pushes**, and **Errors** for clarity.
//!
...
/// LSL builtins module - provides parsing and loading of LSL constants
...
const config = struct {
    ...
    pub const lang: @"build.lua.Language" = .luau; // slua is a luau fork
    pub const luau_use_4_vector: bool = false;
    pub const system_lua: bool = false;
};
...
/// The superset of all errors returned from zslua
...
pub const Lua = opaque {
    ...
    // Minimal runtime state structure for LSL
    // Must match the layout of lua_SLRuntimeState from lua.h:
    // struct lua_SLRuntimeState {
    //     unsigned int slIdentifier;
    //     int uuidWeakTab;
    //     int uuidCompressedWeakTab;
    //     ... (function pointers, but we can leave those null)
    // }
    const RuntimeState = extern struct {
        slIdentifier: c_uint,
        uuidWeakTab: c_int,
        uuidCompressedWeakTab: c_int,
        eventHandlerRegistrationCb: ?*anyopaque,
        setTimerEventCb: ?*anyopaque,
        clockProvider: ?*anyopaque,
        performanceClockProvider: ?*anyopaque,
        randomProvider: ?*anyopaque,
    };
    ...
    pub fn deinit(lua: *Lua) void {
        ...
        // Clean up LSL thread data if it was initialized (slua only)
        if (lang == .luau) {
            const ptr = c.lua_getthreaddata(@ptrCast(lua));
            if (ptr) |runtime_state_ptr| {
                if (data) |a| {
                    a.destroy(@as(*RuntimeState, @ptrCast(@alignCast(runtime_state_ptr))));
                }
            }
        }
    ...
    pub fn initLSLThreadData(lua: *Lua, a: Allocator) !void {
        ...
        const runtime_state = try a.create(RuntimeState);
        runtime_state.* = .{
            .slIdentifier = c.LUA_LSL_IDENTIFIER,
            .uuidWeakTab = -1,
            .uuidCompressedWeakTab = -1,
            .eventHandlerRegistrationCb = null,
            .setTimerEventCb = null,
            .clockProvider = null,
            .performanceClockProvider = null,
            .randomProvider = null,
        };
    ...
    /// Must be called before lua.deinit() if initLSLThreadData was called
    /// Only available in slua (ServerLua/Luau fork)
    pub fn deinitLSLThreadData(lua: *Lua) void {
        if (lang != .luau) @compileError(@src().fn_name ++ " is only available in slua (Luau fork).");
    ...
    /// * The number of results is adjusted to `args.results`, unless `args.results` is `zslua.mult_return`. In this case, all results from the function are pushed
    ...
    /// Sets the current memory category for the Lua state.
    /// Memory categories are used to track memory usage by different components.
    ///
    /// Only available in Luau
    pub fn setMemCat(lua: *Lua, category: i32) void {
        if (lang != .luau) @compileError(@src().fn_name ++ " is only available in Luau.");
        c.lua_setmemcat(@ptrCast(lua), category);
    }

    /// Gets the current memory category for the Lua state.
    ///
    /// Only available in Luau
    pub fn getMemCat(lua: *Lua) i32 {
        if (lang != .luau) @compileError(@src().fn_name ++ " is only available in Luau.");
        return c.lua_getmemcat(@ptrCast(lua));
    }
    ...
    /// Gets a human-readable stack backtrace string for debugging purposes.
    /// Warning: this function is not thread-safe since it stores the result in a shared global array!
    ...
    pub fn debugTrace(lua: *Lua) [:0]const u8 {
        if (lang != .luau) @compileError(@src().fn_name ++ " is only available in Luau.");
        return std.mem.span(c.lua_debugtrace(@ptrCast(lua)));
    }
    ...
    /// Open the LSL (Linden Scripting Language) library
    ///
    /// This provides LSL-specific bindings including tovector(), toquaternion(), uuid(), etc.
    /// Only available in slua (ServerLua)
    ///
    /// * Pops:   `0`
    /// * Pushes: `1` (the lsl module table)
    /// * Errors: `other`
    pub fn openLSL(lua: *Lua) void {
        if (lang != .luau) @compileError(@src().fn_name ++ " is only available in slua (Luau fork).");
        _ = c.luaopen_lsl(@ptrCast(lua));
    }

    /// Open the SL (Second Life) library
    ///
    /// This provides SL-specific types and metatables (UUID, quaternion, vector operators)
    /// Only available in slua (ServerLua)
    ///
    /// * Pops:   `0`
    /// * Pushes: `0`
    /// * Errors: `other`
    pub fn openSL(lua: *Lua, expose_internal_funcs: bool) void {
        if (lang != .luau) @compileError(@src().fn_name ++ " is only available in slua (Luau fork).");
        _ = c.luaopen_sl(@ptrCast(lua), if (expose_internal_funcs) 1 else 0);
    }

    /// Open the ll (Linden Library) functions
    ///
    /// This provides LSL-compatible functions like ll.Sin(), ll.Cos(), ll.List2String(), etc.
    /// Only available in slua (ServerLua)
    ///
    /// * Pops:   `0`
    /// * Pushes: `1` (the ll module table)
    /// * Errors: `other`
    pub fn openLL(lua: *Lua, testing_funcs: bool) void {
        if (lang != .luau) @compileError(@src().fn_name ++ " is only available in slua (Luau fork).");
        _ = c.luaopen_ll(@ptrCast(lua), if (testing_funcs) 1 else 0);
    }

    /// Open the lljson (JSON) library
    ///
    /// This provides JSON encoding/decoding functions via the lljson global.
    /// Functions include lljson.encode(), lljson.decode(), etc.
    /// Only available in slua (ServerLua)
    ///
    /// * Pops:   `0`
    /// * Pushes: `1` (the lljson module table)
    /// * Errors: `other`
    pub fn openLLJson(lua: *Lua) void {
        if (lang != .luau) @compileError(@src().fn_name ++ " is only available in slua (Luau fork).");
    ...
    /// as global variables in the Lua state. Call initLSLBuiltins first to load the constants.
    /// Only available in slua (ServerLua)
    ///
    /// * Pops:   `0`
    /// * Pushes: `0`
    /// * Errors: `never`
    pub fn setLSLConstantGlobals(lua: *Lua) void {
        if (lang != .luau) @compileError(@src().fn_name ++ " is only available in slua (Luau fork).");
        lsl_builtins.setConstantGlobals(lua);
    }

    /// Initialize LSL builtins from embedded data or a file
    ///
    /// If builtins_file is null, uses the embedded builtins data.
    /// This should be called once at startup before using setLSLConstantGlobals.
    /// Only available in slua (ServerLua)
    ///
    /// * Pops:   `0`
    /// * Pushes: `0`
    /// * Errors: `memory`
    pub fn initLSLBuiltins(gpa: std.mem.Allocator, builtins_file: ?[]const u8) !void {
        if (lang != .luau) @compileError(@src().fn_name ++ " is only available in slua (Luau fork).");
        try lsl_builtins.initGlobalBuiltins(gpa, builtins_file);
    }

    /// Deinitialize global LSL builtins and free memory
    /// Only available in slua (ServerLua)
    ///
    /// * Pops:   `0`
    /// * Pushes: `0`
...
// Helper functions to make the zslua API easier to use
