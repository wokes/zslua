...
const Buffer = zslua.Buffer;
const DebugInfo = zslua.DebugInfo;
const Lua = zslua.Lua;
...
/// Return true if zslua.lang matches any of the given langs
inline fn langIn(langs: anytype) bool {
    inline for (langs) |lang| if (zslua.lang == lang) return true;
    return false;
}
...
test "initialization" {
    ...
    try expectEqual(zslua.Status.ok, lua.status());
    lua.deinit();
...
test "Zig allocator access" {
    ...
    lua.pushFunction(zslua.wrap(inner));
    lua.pushInteger(10);
...
test "standard library loading" {
    ...
    // packages into a non-standard table
    {
        ...
        if (zslua.lang != .lua51 and zslua.lang != .lua53 and zslua.lang != .lua54) lua.openBit32();

        if (zslua.lang != .luau) {
            lua.openPackage();
            lua.openIO();
        }
        if (zslua.lang != .lua51 and zslua.lang != .luajit) lua.openCoroutine();
        if (zslua.lang != .lua51 and zslua.lang != .lua52 and zslua.lang != .luajit) lua.openUtf8();
    }
}
...
test "executing string contents" {
    ...
    try expectError(if (zslua.lang == .luau) error.LuaError else error.LuaSyntax, lua.loadString("bad syntax"));
...
test "stack manipulation" {
    ...
    if (zslua.lang == .lua53 or zslua.lang == .lua54) {
        var num: i32 = 1;
        while (num <= 10) : (num += 1) {
            lua.pushInteger(num);
        }
...
test "calling a function" {
    ...
    lua.register("zigadd", zslua.wrap(add));
...
test "calling a function with cProtectedCall" {
    ...
    // sure things work!
    try lua.cProtectedCall(zslua.wrap(testFn), &value);
}

test "version" {
    if (zslua.lang == .lua51 or zslua.lang == .luau or zslua.lang == .luajit) return;
...
test "string buffers" {
    ...
    buffer.addValue();
    buffer.pushResult();
    try expectEqualStrings("zslua api 5.1", try lua.toString(-1));
    ...
    if (zslua.lang == .lua51 or zslua.lang == .luajit) return;
...
test "function registration" {
    ...
    if (langIn(.{ .lua51, .luajit, .luau })) {
        ...
        const funcs = [_]zslua.FnReg{
            .{ .name = "add", .func = zslua.wrap(add) },
        };
        lua.newTable();
...
test "panic fn" {
    ...
    const panicFn = zslua.wrap(struct {
        fn inner(l: *Lua) i32 {
            _ = l;
            return 0;
        }
...
test "warn fn" {
    ...
    const warnFn = zslua.wrap(struct {
...
test "garbage collector" {
    ...
    if (zslua.lang != .lua51 and zslua.lang != .luajit) _ = lua.gcIsRunning();
...
test "extra space" {
    if (zslua.lang != .lua53 and zslua.lang != .lua54) return;
...
test "table access" {
    ...
    try expectEqual(.string, switch (zslua.lang) {
    ...
    while (index <= 5) : (index += 1) {
        ...
        if (zslua.lang == .lua53 or zslua.lang == .lua54) lua.setIndex(-2, index) else lua.rawSetIndex(-2, index);
    }
...
test "conversions" {
    ...
    try expectError(error.LuaError, Lua.numberToInteger(@as(zslua.Number, @floatFromInt(zslua.max_integer)) + 10));
...
test "dump and load" {
    ...
    if (zslua.lang == .lua53 or zslua.lang == .lua54) {
        try lua.dump(zslua.wrap(writer), &buffer, false);
    } else {
        try lua.dump(zslua.wrap(writer), &buffer);
    }
    ...
    if (zslua.lang == .lua51 or zslua.lang == .luajit) {
        try lua.load(zslua.wrap(reader), &buffer, "function");
    } else {
        try lua.load(zslua.wrap(reader), &buffer, "function", .binary);
    }
    try expectEqual(.function, lua.typeOf(-1));
...
test "userdata and uservalues" {
    ...
    var data = if (zslua.lang == .lua54) lua.newUserdata(Data, 2) else lua.newUserdata(Data);
    data.val = 1;
    ...
    if (zslua.lang == .lua52 or zslua.lang == .lua53) {
...
test "upvalues" {
    ...
    lua.pushInteger(0);
    lua.pushClosure(zslua.wrap(counter), 1);
...
test "registry" {
    ...
    lua.rawSetPtr(zslua.registry_index, key);

    // get key from the registry
    _ = lua.rawGetPtr(zslua.registry_index, key);
...
test "raise error" {
    ...
    lua.pushFunction(zslua.wrap(makeError));
    try expectError(error.LuaRuntime, lua.protectedCall(.{}));
    try expectEqualStrings("makeError made an error", try lua.toString(-1));
}

fn continuation(l: *Lua, status: zslua.Status, ctx: isize) i32 {
    _ = status;
    ...
    if (ctx == 5) {
        ...
        l.pushInteger(ctx);
        return l.yieldCont(1, ctx + 1, zslua.wrap(continuation));
    }
}
...
test "yielding" {
    ...
    var thread = lua.newThread();
    thread.pushFunction(zslua.wrap(willYield));
...
fn continuation52(l: *Lua) !i32 {
    ...
    if (ctx == 5) {
        ...
        // yield the current context value
        l.pushInteger(ctx);
        return l.yieldCont(1, ctx + 1, zslua.wrap(continuation52));
    }
}
...
test "yielding no continuation" {
    if (zslua.lang != .lua51 and zslua.lang != .luau) return;

    const lua: *Lua = try .init(testing.allocator);
    defer lua.deinit();

    var thread = lua.newThread();
    const func = zslua.wrap(struct {
        fn inner(l: *Lua) i32 {
            l.pushInteger(1);
            return l.yield(1);
        }
...
test "resuming" {
    ...
    var i: i32 = 1;
    while (i <= 5) : (i += 1) {
        try expectEqual(.yield, if (zslua.lang == .lua51 or zslua.lang == .luajit) try thread.resumeThread(0) else try thread.resumeThread(lua, 0));
        try expectEqual(i, thread.toInteger(-1));
        lua.pop(lua.getTop());
    }
    try expectEqual(.ok, if (zslua.lang == .lua51 or zslua.lang == .luajit) try thread.resumeThread(0) else try thread.resumeThread(lua, 0));
    try expectEqualStrings("done", try thread.toString(-1));
}
...
test "aux check functions" {
    ...
    const function = zslua.wrap(struct {
        fn inner(l: *Lua) i32 {
            l.checkAny(1);
            _ = l.checkInteger(2);
            _ = l.checkNumber(3);
            ...
            _ = if (zslua.lang == .lua52) l.checkUnsigned(6);
            return 0;
        }
    }.inner);
    ...
    if (zslua.lang == .lua54) lua.pushFail() else lua.pushNil();
    lua.pushInteger(3);
    lua.pushNumber(4);
    _ = lua.pushString("hello world");
    lua.pushBoolean(true);
...
test "loadBuffer" {
    ...
    try lua.protectedCall(.{ .args = 0, .results = zslua.mult_return });
...
test "where" {
    ...
    const whereFn = zslua.wrap(struct {
        fn inner(l: *Lua) i32 {
            l.where(1);
            return 1;
        }
...
test "ref" {
    ...
    lua.pushNil();
    try expectError(error.LuaError, lua.ref(zslua.registry_index));
    try expectEqual(0, lua.getTop());

    _ = lua.pushString("Hello there");
    const ref = try lua.ref(zslua.registry_index);

    _ = lua.rawGetIndex(zslua.registry_index, ref);
    try expectEqualStrings("Hello there", try lua.toString(-1));

    lua.unref(zslua.registry_index, ref);
}
...
test "metatables" {
    ...
    if (!langIn(.{ .lua51, .luajit, .luau })) {
        ...
        _ = lua.getField(zslua.registry_index, "mt");
        lua.setMetatable(-2);
    }
...
test "args and errors" {
    ...
    const argCheck = zslua.wrap(struct {
        fn inner(l: *Lua) i32 {
    ...
    const raisesError = zslua.wrap(struct {
        fn inner(l: *Lua) i32 {
    ...
    const argExpected = zslua.wrap(struct {
        fn inner(l: *Lua) i32 {
...
test "traceback" {
    ...
    const tracebackFn = zslua.wrap(struct {
        fn inner(l: *Lua) i32 {
            l.traceback(l, "", 1);
            return 1;
        }
...
test "userdata" {
    ...
    const checkUdata = zslua.wrap(struct {
        fn inner(l: *Lua) i32 {
    ...
    {
        var t = if (zslua.lang == .lua54) lua.newUserdata(Type, 0) else lua.newUserdata(Type);
        if (langIn(.{ .lua51, .luajit, .luau })) {
            _ = lua.getField(zslua.registry_index, "Type");
            lua.setMetatable(-2);
    ...
    const testUdata = zslua.wrap(struct {
        fn inner(l: *Lua) !i32 {
...
test "userdata slices" {
    ...
    const slice = if (zslua.lang == .lua54) lua.newUserdataSlice(Integer, 10, 0) else lua.newUserdataSlice(Integer, 10);
    if (langIn(.{ .lua51, .luajit, .luau })) {
        _ = lua.getField(zslua.registry_index, "FixedArray");
        lua.setMetatable(-2);
    ...
    lua.pushFunction(zslua.wrap(udataFn));
    lua.pushValue(2);
...
test "debug interface" {
    ...
    // create a hook
    const hook = struct {
        fn inner(l: *Lua, event: zslua.Event, i: *DebugInfo) !void {
            switch (event) {
                .call => {
                    if (zslua.lang == .lua54) l.getInfo(.{ .l = true, .r = true }, i) else l.getInfo(.{ .l = true }, i);
                    if (i.current_line.? != 2) std.debug.panic("Expected line to be 2", .{});
                    _ = if (zslua.lang == .lua54) try l.getLocal(i, i.first_transfer) else try l.getLocal(i, 1);
    ...
    try expectEqual(zslua.HookMask{}, lua.getHookMask());
    try expectEqual(0, lua.getHookCount());

    lua.setHook(zslua.wrap(hook), .{ .call = true, .line = true, .ret = true }, 0);
    try expectEqual(zslua.wrap(hook), lua.getHook());
    try expectEqual(zslua.HookMask{ .call = true, .line = true, .ret = true }, lua.getHookMask());
...
test "debug interface Lua 5.1 and Luau" {
    ...
    // run the hook when a function is called
    try expectEqual(@as(?zslua.CHookFn, null), lua.getHook());
    try expectEqual(zslua.HookMask{}, lua.getHookMask());
    try expectEqual(@as(i32, 0), lua.getHookCount());

    lua.setHook(zslua.wrap(hook), .{ .call = true, .line = true, .ret = true }, 0);
    try expectEqual(@as(?zslua.CHookFn, zslua.wrap(hook)), lua.getHook());
    try expectEqual(zslua.HookMask{ .call = true, .line = true, .ret = true }, lua.getHookMask());
...
test "debug upvalues" {
    ...
    try expectEqualStrings(if (zslua.lang == .luau) "" else "x", try lua.getUpvalue(-1, 1));
    try expectEqual(1, try lua.toNumber(-1));
    lua.pop(1);
    ...
    if (zslua.lang == .lua54) try expectError(error.LuaError, lua.upvalueId(-1, 2));
...
test "getstack" {
    ...
    lua.pushFunction(zslua.wrap(function));
    lua.setGlobal("f");
...
test "compile and run bytecode" {
    ...
    const bc = try zslua.compile(testing.allocator, src, zslua.CompileOptions{});
    ...
    const bc1 = try zslua.compile(testing.allocator, src2, zslua.CompileOptions{});
    defer testing.allocator.free(bc1);

    const options = zslua.CompileOptions{
        .mutable_globals = &[_:null]?[*:0]const u8{ "Foo", "Bar" },
    };
    const bc2 = try zslua.compile(testing.allocator, src2, options);
...
test "userdata dtor" {
    ...
    // create a Lua-owned pointer to a Data, configure Data with a destructor.
    {
        ...
        var data = lua.newUserdataDtor(Data, zslua.wrap(Data.dtor));
...
test "luau vectors" {
    ...
    defer lua.deinit();
    lua.openLibs();
    lua.register("vector", zslua.wrap(vectorCtor));
    ...
    if (zslua.luau_vector_size == 3) lua.pushVector(1, 2, 3) else lua.pushVector(1, 2, 3, 4);
    try expect(lua.isVector(-1));
    v = try lua.toVector(-1);
    const expected = if (zslua.luau_vector_size == 3) [3]f32{ 1, 2, 3 } else [4]f32{ 1, 2, 3, 4 };
...
test "useratom" {
    ...
    const lua: *Lua = try .init(testing.allocator);
    defer lua.deinit();
    lua.setUserAtomCallbackFn(zslua.wrap(useratomCb));
...
test "namecall" {
    ...
    const lua: *Lua = try .init(testing.allocator);
    defer lua.deinit();
    lua.setUserAtomCallbackFn(zslua.wrap(funcs.useratomCb));
    ...
    lua.pushFunctionNamed(zslua.wrap(funcs.vectorNamecall), "vector_namecall");
    lua.setTable(-3);
...
test "define" {
    ...
    var state: zslua.def.DefineState = .empty;
    defer state.deinit(a);

    const to_define: []const type = &.{ T, TestType, Foo };
    inline for (to_define) |my_type| {
        _ = try zslua.def.addClass(&state, a, my_type);
    }
...
test "interrupt" {
    ...
    lua.setInterruptCallbackFn(zslua.wrap(interrupt_handler.inner));
...
test "error union for CFn" {
    ...
    lua.pushFunction(zslua.wrap(fails));
...
test "checkNumeric and toNumeric" {
    ...
    lua.pushFunction(zslua.wrap(struct {
        fn f(l: *Lua) i32 {
...
test "function registration with fnRegsFromType" {
    ...
    if (zslua.lang == .lua51 or zslua.lang == .luau or zslua.lang == .luajit) {
        const funcs = comptime zslua.fnRegsFromType(MyLib);
        lua.newTable();
        lua.registerFns("fnregs", funcs);
    } else {
        lua.newLib(comptime zslua.fnRegsFromType(MyLib));
